...
fname_json <- paste0(sample_name,"_", genome_assembly,".cacao.json")
...
for(c in c('hereditary','somatic_actionable','somatic_hotspot')){
  if(tsv_annotation_fname[[c]] != "NA" && file.exists(tsv_annotation_fname[[c]])){
    ...
    if(nrow(coverage_pr_loci[[c]]) > 0 && "CALLABILITY" %in% colnames(coverage_pr_loci[[c]])){
      ...
      cacao_report[['global_distribution']][[c]] <- cacao::global_coverage_distribution(cacao_report[['loci']][[c]][['all']])
      cacao_report[['gene_distribution']][[c]] <- cacao::coverage_distribution_pr_gene(coverage_pr_loci, mode = c)
    }
  }
}
rlogging::message('------')
...
rmarkdown::render(system.file("templates","cacao_report.Rmd", package="cacao"), output_format = rmarkdown::html_document(theme = "default", toc = T, toc_depth = 3, toc_float = T, number_sections = F), output_file = paste0(sample_name, "_", genome_assembly, ".cacao.html"), output_dir = out_dir, clean = T, intermediates_dir = out_dir, quiet = T)
