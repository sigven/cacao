...
## Hereditary cancer - known pathogenic loci
...
## Actionable loci - somatic mutations

### Global distribution

```{r global_dist_somatic_actionable, echo = F, fig.width=12, fig.height=2, eval=T}
...
```{r somatic_actionable_loci, echo=F, results = 'asis', eval = eval_somatic_actionable_loci}
library(summarywidget)
library(crosstalk)

loci_sa <- crosstalk::SharedData$new(cacao_report[['loci']][['somatic_actionable']][['all']])
crosstalk::bscols(
  list(
    crosstalk::filter_select("CALLABILITY", "Callability", loci_sa, ~CALLABILITY),
    crosstalk::filter_select("SYMBOL", "Gene",loci_sa, ~SYMBOL),
    crosstalk::filter_select("NAME", "Locus name", loci_sa, ~NAME)
  ),
  list(
    crosstalk::filter_select("CANCERTYPE", "Cancer type",loci_sa, ~CANCERTYPE),
    crosstalk::filter_select("EVIDENCE_TYPE", "Type of evidence", loci_sa, ~EVIDENCE_TYPE),
    crosstalk::filter_select("EVIDENCE_LEVEL", "Level of evidence", loci_sa, ~EVIDENCE_LEVEL),
    crosstalk::filter_select("THERAPEUTIC_CONTEXT", "Therapeutic context", loci_sa, ~THERAPEUTIC_CONTEXT)
  )
)

crosstalk::bscols(
  DT::datatable(loci_sa, escape=F,extensions=c("Buttons","Responsive"), width = "100%",options=list(buttons = c('csv','excel'),dom = 'Bfrtip')) %>%
  DT::formatStyle('CALLABILITY', backgroundColor = DT::styleEqual(c('NO_COVERAGE','LOW_COVERAGE','CALLABLE','HIGH_COVERAGE'), c("#FC4E2A","#FD8D3C","#78C679","#41AB5D")))
)
...
```{r global_dist_somatic_hotspots, echo = F, fig.width=12, fig.height=2, eval=T}
